# Enterprise Features

This index groups the enterprise-grade capabilities delivered in v2.0.0 and planned for later
major releases.

- [v2.0.0 Enterprise Release](Release-v2.0.0)
- [v3.0.0 Ecosystem Release](Release-v3.0.0)
- [v4.0.0 Intelligent Release](Release-v4.0.0)
- [Roadmap](Roadmap)
- [How-To Guides](How-To-Guides)

## Governance & Controls

- [Policy Authoring UI](Policy-Authoring-UI.md)
- [Policy Layering Model](Policy-Layering.md)
- [Org-Wide Baselines](Org-Wide-Baselines.md)
- [Group-Level Enforcement](Group-Level-Enforcement.md)
- [Policy Metadata + Rich Failure Messages](Policy-Metadata.md)
- [Policy Versioning + Audit Trail](Policy-Registry.md)

## Compliance & Evidence

- [Evidence Export](Evidence-Export.md)
- [Audit Trails](Policy-Registry.md)

## Runtime Enforcement

- [Drift Prevention](Drift-Prevention.md)
- [Guardrails-as-a-Service API](Guardrails-as-a-Service.md)
- [Context-Aware Evaluation](Context-Aware-Evaluation.md)
- [Suggested Fixes](Suggested-Fixes.md)
- [Explainability Reports](Explainability-Reports.md)
- [Enterprise SARIF/JUnit](Enterprise-SARIF-JUnit.md)
- [Policy Waivers](Policy-Waivers.md)

## Integrations & Delivery

- [GitHub Action](GitHub-Action.md)
- [GitLab CI Templates](GitLab-CI.md)
- [Azure DevOps Extension](Azure-DevOps.md)
- [AWS CodePipeline](AWS-CodePipeline.md)

## Platform & Packaging

- [Docker Compose Stack](Docker-Compose.md)
- [Packaging](Packaging.md)
- [Policy Packs](Policy-Packs.md) - delivered in v3.0 development
